	...
	if (Settings::Misc::misc_SkinChanger && bIsGuiVisible)
	{
			...
			ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize))
		{
			...
			if (SafeWeapon != 0)
			{
				...
				ImGui::Combo("Stiker", &Stick[SafeWeapon].Stickers[iSlot].iID, Stikers, IM_ARRAYSIZE(Stikers));
			}
			ImGui::Spacing();
			ImGui::Separator();
			ImGui::Spacing();
