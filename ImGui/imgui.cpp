       ...
       for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          ...
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 MyEngineScissor((int)cmd->ClipRect.x, (int)cmd->ClipRect.y, (int)(cmd->ClipRect.z - cmd->ClipRect.x), (int)(cmd->ClipRect.w - cmd->ClipRect.y));

                 // Render 'cmd->ElemCount/3' indexed triangles.
                 ...
                 MyEngineDrawIndexedTriangles(cmd->ElemCount, sizeof(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, idx_buffer, vtx_buffer);
             }
...
void ImGui::Render()
{
    ...
    if (g.Style.Alpha > 0.0f)
    {
        ...
        if (g.IO.MouseDrawCursor)
        {
            ...
            g.OverlayDrawList.AddImage(tex_id, pos,             pos + size,             cursor_data.TexUvMin[0], cursor_data.TexUvMax[0], IM_COL32(0,60,255,255)); // White fill
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (*v)
	{
		window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y), check_bb.Max, GetColorU32(ImGuiCol_CheckMark), 0, -2);
		window->DrawList->AddRect(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y), check_bb.Max, GetColorU32(ImGuiCol_Border), 0, -3, 4);
	}
	else
	{
    ...
    if (ImGui::Begin("ImGui Metrics", p_open))
    {
        ...
        struct Funcs
        {
            static void NodeDrawList(ImDrawList* draw_list, const char* label)
            {
                ...
                int elem_offset = 0;
                for (const ImDrawCmd* cmd = draw_list->CmdBuffer.begin(); cmd < draw_list->CmdBuffer.end(); elem_offset += cmd->ElemCount, cmd++)
                {
                    if (cmd->UserCallback == NULL && cmd->ElemCount == 0)
                        continue;
                    if (cmd->UserCallback)
                    {
                        ImGui::BulletText("Callback %p, user_data %p", cmd->UserCallback, cmd->UserCallbackData);
                        continue;
                    }
                    ImDrawIdx* idx_buffer = (draw_list->IdxBuffer.Size > 0) ? draw_list->IdxBuffer.Data : NULL;
                    bool cmd_node_open = ImGui::TreeNode((void*)(cmd - draw_list->CmdBuffer.begin()), "Draw %-4d %s vtx, tex = %p, clip_rect = (%.0f,%.0f)..(%.0f,%.0f)", cmd->ElemCount, draw_list->IdxBuffer.Size > 0 ? "indexed" : "non-indexed", cmd->TextureId, cmd->ClipRect.x, cmd->ClipRect.y, cmd->ClipRect.z, cmd->ClipRect.w);
                    if (show_clip_rects && ImGui::IsItemHovered())
                    {
                        ImRect clip_rect = cmd->ClipRect;
                        ImRect vtxs_rect;
                        for (int i = elem_offset; i < elem_offset + (int)cmd->ElemCount; i++)
                    ...
                    ImGuiListClipper clipper(cmd->ElemCount/3); // Manually coarse clip our print out of individual vertices to save CPU, only items that may be visible.
                    while (clipper.Step())
