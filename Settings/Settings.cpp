...
namespace Settings
{
	void LoadSettings(string szIniFile)
	{
		...
		for (DWORD i = 0; i < WEAPON_DATA_SIZE; i++)
		{
			...
			if (i <= 9)
			{
				...
				Aimbot::weapon_aim_settings[i].aim_Rcsx = CSX::Cvar::LoadCvar(pWeaponData[i], CVAR_AIMBOT_RCSX, 1);
				Aimbot::weapon_aim_settings[i].aim_Rcsy = CSX::Cvar::LoadCvar(pWeaponData[i], CVAR_AIMBOT_RCSY, 1);
		...
		Esp::esp_InScoped = CSX::Cvar::LoadCvar(VISUAL_TEXT, CVAR_ESP_SCOPE, Esp::esp_InScoped) != false;
	...
	void SaveSettings(string szIniFile)
	{
		...
		CSX::Cvar::SaveCvar(VISUAL_TEXT, CVAR_ESP_SCOPE, Settings::Esp::esp_InScoped);
